...
`trips.json` has a top-level **`party_configs`** array; every trip is scanned once per config. Each config: `{id, label, short, adults, children:[ages], nights, auto}`. Current: `couple` (2 adults, 2 nights, **auto**), `family` (4 adults + child age 3, 3 nights, manual), `six` (6 adults + child age 3, 3 nights, manual).
- **`auto` flag (user decision 2026-07-27)**: only `auto: true` configs run in the daily Action. `auto: false` ones run **only on demand**: `python travel_tracker.py --party family` (accepts a comma list, or `--party all`). An explicit `--party` overrides the flag.
- **Carry-forward**: when a new day starts and today's snapshot doesn't exist yet, `main()` seeds the snapshot from the most recent previous file — otherwise the daily couple-only run would erase family/six from the dashboard. Each party snap stores `scanned` (date) and `auto`; the dashboard shows a staleness banner (amber after 3 days) for manual parties.
